...
# Load datasets
medal_counts = pd.read_csv('data/summerOly_medal_counts.csv', encoding='latin1')
hosts = pd.read_csv('data/summerOly_hosts.csv', encoding='latin1')
programs = pd.read_csv('data/summerOly_programs.csv', encoding='latin1')
athletes = pd.read_csv('data/summerOly_athletes.csv', encoding='latin1')
...
medal_counts['Host'] = medal_counts['Year'].map(
    hosts.set_index('ï»¿Year')['Host'].to_dict()
)
...
plt.figure(figsize=(10, 5))
plt.bar(features, importance_gold, color='yellow')
plt.xlabel('Features')
...
plt.figure(figsize=(10, 5))
plt.bar(features, importance_total, color='blue')
plt.xlabel('Features')
plt.ylabel('Importance')
